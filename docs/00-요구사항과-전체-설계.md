...
전체 서비스의 최종 성능 목표는 **10,000 TPS**입니다. 전반적인 개발을 완료한 뒤 성능 튜닝·설정을 진행하고, 서비스별 Pod 1개당 최대 지속 성능과 전체 확장 성능을 측정합니다. [목표·진행 순서·측정 계획](41-전체-만-TPS-목표와-Pod별-성능-검증-계획.md)을 따릅니다.
...
본격 성능 검증은 전반적인 개발 완료와 초기 튜닝·설정 이후 진행합니다. 서비스별 Pod 한계를 측정한 뒤 전체 입력을 점진적으로 **목표 10,000 TPS**까지 올리고 각 단계에서 아래를 함께 기록합니다. 아래 단계별 수치와 기존 로컬 PoC는 목표 달성의 증거가 아닙니다.
