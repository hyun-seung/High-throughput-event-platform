...
물리 테이블은 `ORIGIN`과 `STEP`이다. [정상·복잡한 경로의 단계별 합산표](36-원본과-단계-테이블-분리와-경로별-쓰기-내역.md)에서 각각의 insert/update/delete와 테이블별 합계를 설명한다. ‘7항목’은 서로 다른 7개 행이 아니라 같은 행에 대한 변경을 합한 7회라는 뜻이다.
