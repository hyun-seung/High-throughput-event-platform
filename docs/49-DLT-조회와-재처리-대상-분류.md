...
2026-09-25. DLT 후속 작업의 첫 단계로 **Java 읽기 전용 조회·분류 CLI**를 추가했다. 후속 [원장 대조와 복구](50-DLT-원장-대조와-기존-실행-복구.md)에서 기존 실행 재개, 이력 보존 범위 내 초기 원본 복원·만료 인계와 SQL 조치 기록을 추가했다. 조회 성공이나 REQUIRES_STATE_CHECK 분류를 재발송 승인으로 해석하지 않는다.
