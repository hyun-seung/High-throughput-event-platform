...
- Mock Provider는 deliveryId를 Idempotency-Key로 받아 동일 key의 부수 효과를 한 번으로 수렴시킵니다.
...
| Dispatch Consumer | consume 1건 | 아직 없음 |
...
| Dispatch 멱등성 | Provider key만 사용 | `DISPATCH_ATTEMPT` claim/lease/완료 상태 | P0 |
| 결과 불명 | 없음 | timeout/reset을 `UNKNOWN`으로 저장하고 조정 | P0 |
...
Provider 호출은 Mock Provider가 idempotency key를 지원하는 경우에만 실제 부수 효과가 한 번으로 수렴합니다. Dispatch Worker 자체의 claim과 결과 저장은 아직 없으므로 이를 전체 Provider에 대한 exactly-once 보장으로 표현하지 않습니다.
