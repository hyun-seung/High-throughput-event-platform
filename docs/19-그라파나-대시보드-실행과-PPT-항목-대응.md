...
> Kafka 기존 환경 갱신: 실제 로그 경로의 명명 볼륨 마운트를 수정했습니다. 기존 부모 경로 마운트를 쓰는 컨테이너는 재생성 전에 [데이터 확인·이관 절차](47-Kafka-장애-시-접수-보류와-결과-복구-검증.md#3-시험-중-발견한-kafka-데이터-마운트)를 따릅니다. 실행 스크립트는 이전 구조를 발견하면 데이터 변경 없이 중단합니다.
...
현재 업무 코드는 HTTP·TCP 발송, 업체 웹훅, 만료·고객 통지까지 구현돼 있다. 대시보드 파일에도 HTTP 접수 외에 TCP 접수·웹훅 처리 패널을 추가했다. **업무 코드 구현, 패널 정의, 실행 중인 앱의 지표 수집은 별도 상태**다. 전체 후속 단계 수집·패널·경보 연결이 완료된 것은 아니며, 아래 초기 구축 표의 미구현 표시는 당시 범위다. 예시 수치나 가상의 통신사·AWS 존을 정상 상태처럼 넣지 않는다. 최신 기능 상태는 [01 진행 현황](01-현재-구현-상태와-남은-작업.md), 추가한 패널의 검증 범위는 이 문서 하단을 참고한다.
...
다음 표는 최초 화면 구성 시점의 대응 기록이다. 이후 TCP·웹훅 패널 추가는 하단에 기록했으며, 업무 기능의 최신 완료 여부는 이 표로 판단하지 않는다.
...
웹훅 처리 TPS·Attempt 성공 반영 TPS·늦은 웹훅 폐기·결과 트랜잭션 p95 패널을 추가해 현재 파일은 **7개 화면·96개 패널·84개 질의**다. 4개 신규 PromQL은 기존 Prometheus에서 성공 응답을 확인했다. 기존 앱은 이전 JAR이므로 시계열은 0개이며 새 빌드 배포가 필요하다. Grafana 화면 provisioning은 위 런타임 문제로 아직 검증하지 않았다. [결과 처리와 관측의 의미](27-웹훅-결과-반영과-재시도-대체-발송-복구.md).
