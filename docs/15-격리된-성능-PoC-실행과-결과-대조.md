...
## Grafana 환경에서 같은 정합성 검증 실행

이미 실행 중인 `platform-monitoring`의 앱·관측 서버를 유지한 채 시험하려면 `bash scripts/monitoring.sh benchmark --suite baseline`을 사용한다. 관리 포트를 추가 공개하지 않고 기존 collector 내부에서 지표와 시험용 업체 횟수를 읽는다. 기준선의 요청별 대조 로직과 중단 조건을 재사용하며 앱·정책·인프라 수명 관리는 분리한다. [실행 절차](19-그라파나-대시보드-실행과-PPT-항목-대응.md)와 [측정 기록](20-모니터링-동시-실행-성능-검증.md)을 참고한다.
