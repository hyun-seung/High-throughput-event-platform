...
이 문서는 `715c99d` 당시 수신 기능과 시험 기록이다. 후속으로 invocation 선택 필드와 [Attempt 결과 반영·후속 발송 복구](27-웹훅-결과-반영과-재시도-대체-발송-복구.md)를 구현했다. 새 웹훅은 발송 요청의 invocation을 돌려주며, 기존 필드 생략은 최초 호출만 처리 가능하다. 아래 미구현 설명은 당시 범위이며 최신 상태는 후속 문서를 따른다.
