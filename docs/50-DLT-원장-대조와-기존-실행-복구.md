...
최신 실행 결과는 **전체 413개 통과, 실패·오류·미실행 0개**다. 이번 장애 호출 제한은 12개 시험을 추가했으며 그중 DLT 통합 시험은 3개다. [검증 결과 JSON](검증-결과/2026-09-26-SQL과-DLT-장애-호출-제한-413개-통합-검증.json)에 도구·실행 폴더·소스 지문·정리 결과와 개발 중 실패 원인을 남겼다. 기존 운영 조회·감사 재검토 8개 시험의 최초 기록은 [356개 검증](검증-결과/2026-09-26-DLT-운영-조회와-감사-재검토-356개-통합-검증.json)이다.
...
- SQL 연결 오류·Kafka timeout을 Java에서 주입하고 단조 시계를 진행해 호출 제한을 검증. 100회 반복 호출 생략, 반환된 UNCONFIRMED에서도 묶음 중단, 복구 확인 1건 선택, 실제 SQL 원문·cursor·기존 command와 원래 시각 보존 후 Kafka 인계 확인. 실제 네트워크 단절 시험과 구분한다.

실제 SIGKILL·SQL commit 응답 유실을 포함한 모든 종료 지점을 이 시험이 검증한 것은 아니다. 성능/TPS 시험도 아니며, 성능 부하는 개발 완료 후 **k6**로 진행한다.

보류 메타데이터·사유 조회와 감사 재검토는 로컬 CLI로 구현했다. 결과 불명·통지 소진·정리 대기 조회, 결과 불명 확정과 정리 재조치도 후속 구현됐다. 운영 권한·통지 소진 후 추가 조치·경보 연결은 [현재 진행 현황](01-현재-구현-상태와-남은-작업.md)에서 관리한다. 이력 보존 범위 밖의 요청은 여전히 보류한다. 현재 복구 도구와 Lifecycle만으로 모든 DLT의 고객 결과 기한 보장이 완료됐다고 보지 않는다.
...
# 같은 조건으로 계속 실행. 각 묶음 뒤 최소 30초, 장애 시 남은 백오프가 더 길면 그만큼 대기
...
SQL 연결·시간 초과/과부하, DynamoDB 연결·5xx/throttling, Kafka 재시도 가능 오류·인계 timeout을 감지하면 첫 실패에서 현재 묶음을 멈춘다. `DltRecoveryStore`가 오류를 `UNCONFIRMED`로 반환하는 경로도 `backendUnavailable=true`를 함께 반환해 제한기에 전달한다. 이 값은 해당 호출의 관찰이며 저장된 원장의 새 상태가 아니다. SQL 데이터 충돌·잘못된 원문·조건 불일치는 별도 기존 판단을 유지한다.

기본 지연 상한은 1→2→4→8→16→30초이며 매번 그 값의 50~100%를 기다린다. `DLT_FAILURE_INITIAL_DELAY_MS` / `DLT_FAILURE_MAX_DELAY_MS`로 변경한다. 기존 주기와 남은 백오프 중 긴 시간을 기다리고, 이후 재개 worker는 최대 1건을 선택한다. 성공하면 설정된 묶음 크기로 돌아간다. 기존 minimumAge를 줄이거나 command·시도 이력·deadline을 초기화하지 않는다.

| 출력 | 의미 |
|---|---|
| `SCANNED` / `CURSOR_MOVED` | 기존 정상 조회/다른 작업의 cursor 진행. 개별 결과·적체도 함께 확인 |
| `BACKEND_UNAVAILABLE` | 이번 주기에 기반 저장소 장애 관찰. 처리한 앞부분 결과만 반환하며 나머지는 저장된 상태로 대기 |
| `BACKOFF` | 이번 호출에서 저장소를 조회하지 않음. intake offset `-1`·backlog `null`은 **미관찰**이며 커서 소실·적체 0이 아님 |
| `INTAKE_DEFERRED` | 인계 단계가 장애/대기 중이어서 같은 CLI 주기의 PENDING 재개도 생략 |

인계와 재개는 독립된 제한기를 사용한다. 정상 원문 조회만으로 재개의 장애 지연을 초기화하지 않는다. 원문·cursor가 이미 commit된 뒤 인계가 실패한 경우에도 그 위치를 되돌리지 않고 SQL 원문·PENDING 체크포인트로 이어간다. 실패 시 적체 조회를 추가로 시도하지 않으므로 `BACKEND_UNAVAILABLE`의 backlog도 `null`이다. `resume-once`/`intake-once`는 대기·장애를 빈 성공으로 취급하지 않고 3을 반환하며, 호출 밖으로 전파된 저장소 예외는 기존 1을 유지한다.

제한기는 프로세스 메모리 상태이며 재시작하면 초기화된다. 다중 프로세스 공통 제한·클라이언트 내부 reconnect 설정·단발 plan/apply 호출 제한은 구현 범위가 아니다. 알려진 보관 공백의 중단 동작도 유지한다.
