...
> 2026-09-26: [조회·분류 CLI](49-DLT-조회와-재처리-대상-분류.md)에 이어 [원장 대조·복구 CLI](50-DLT-원장-대조와-기존-실행-복구.md)를 구현했습니다. 미등록 DLT 원문과 조회 위치를 SQL에 원자 저장하고, 적격 건의 발송·만료 인계와 중단된 조치 재개를 자동 수행합니다. 보류 건은 사유·원문을 보존하며 초기 원본 복구는 기본 비활성화입니다.
...
로컬 수동 CLI는 기존 실행을 재개하고, 활성화한 이력 보존 범위에서 초기 원본도 복원한다. Java intake worker는 지정 partition의 DLT를 별도 SQL cursor로 읽어 원문을 먼저 보존하고 기존 복구 경로로 인계한다. Kafka 업무 group offset은 변경하지 않는다. 저장된 PENDING 조치도 자동 재개하며 실제 명령·보류 기준은 [50번 문서](50-DLT-원장-대조와-기존-실행-복구.md)를 따른다.
...
3. 원래 인입 시각과 ORIGIN·STEP·SQL 이력을 확인한다. ORIGIN·이력 없음만으로는 미발송으로 단정하지 않는다. 초기 복구가 활성화되고 이력 보존 범위 안인 경우에만 보류 원본을 예약하고 SQL을 다시 대조해 복원한다. 기한이 지난 요청은 원래 기한 그대로 dispatch/lifecycle의 만료·대체 경로로 인계한다.
4. 원장과 정확히 일치하고 STEP이 없는 대상은 **ORIGIN의 기존 실행 ID**로 DispatchRequested를 만든다. 접수 토픽에 원문을 재발행하면 완료 후 정리 경합에서 새 실행을 만들 수 있으므로 사용하지 않는다. requestKey·원래 인입 시각·payload·대체 설정을 유지한다.
5. SQL에 인계 요청·시도를 먼저 기록하고 현재 상태를 재확인한 뒤 Kafka로 보낸다. ack와 SQL 완료 기록까지 확인한 경우만 인계 완료다. 응답 미확인 시 같은 명령을 남기며 격리 원본을 삭제하지 않는다. 업무 group offset을 임의로 변경하지 않는다.
