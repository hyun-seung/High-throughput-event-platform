...
> 2026-09-26: [조회·분류 CLI](49-DLT-조회와-재처리-대상-분류.md)에 이어 [원장 대조·복구 CLI](50-DLT-원장-대조와-기존-실행-복구.md)를 구현했습니다. 기존 실행 재개와 이력 보존 범위 내 초기 원본 복원·만료 인계를 수동으로 수행하고 SQL에 조치를 남깁니다. 초기 복구는 기본 비활성화이며, 자동 DLT 처리는 후속입니다.
...
로컬 수동 CLI는 기존 실행을 재개하고, 명시적으로 활성화한 이력 보존 범위에서 초기 원본도 복원한다. 이미 SQL에 체크포인트를 남긴 미완료 조치는 Java worker가 자동 재개할 수 있다. 미등록 DLT 전체를 읽는 replay Consumer는 아직 없다. 실제 명령·보류 기준은 [50번 문서](50-DLT-원장-대조와-기존-실행-복구.md)를 따른다.
