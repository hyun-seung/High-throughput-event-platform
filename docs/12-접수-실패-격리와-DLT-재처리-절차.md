...
> 2026-09-25: [조회·분류 CLI](49-DLT-조회와-재처리-대상-분류.md)에 이어 [원장 대조·기존 실행 복구 CLI](50-DLT-원장-대조와-기존-실행-복구.md)를 추가했습니다. ORIGIN이 남은 실행을 SQL에 조치 기록 후 dispatch 토픽으로 인계합니다. ORIGIN 없는 요청의 복원·만료 인계와 자동 DLT 처리는 후속입니다.
...
로컬 수동 CLI는 ORIGIN에 남은 기존 실행만 재개한다. 자동 DLT replay Consumer는 아직 없다. 실제 명령·보류 기준은 [50번 문서](50-DLT-원장-대조와-기존-실행-복구.md)를 따른다.
...
3. 원래 인입 시각과 ORIGIN·STEP·SQL 이력을 확인한다. ORIGIN·이력 없음은 미발송 증거로 단정하지 않고 보류한다. 기한이 지난 기존 실행은 원래 기한 그대로 dispatch/lifecycle의 만료·대체 경로로 인계한다.
