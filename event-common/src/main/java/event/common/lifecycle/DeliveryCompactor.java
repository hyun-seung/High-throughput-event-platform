...
public class DeliveryCompactor {
    ...
    public DeliveryCompactor(DynamoDbClient db) { this(db, event.common.redis.DeliveryCache.UNAVAILABLE); }
    public DeliveryCompactor(DynamoDbClient db, event.common.redis.DeliveryCache cache) { this.db = db; this.cache = cache; }

    public boolean compact(DeliveryFinalized result) {
        var meta = read(result.requestKey(), "META");
        if (result.schemaVersion() >= 2 && (meta.isEmpty() || !result.deliveryId().equals(value(meta, DELIVERY_ID)))) {
            // A prior cleanup committed, possibly followed by an allowed new generation. Never touch its origin/cache.
            if (!read(result.deliveryId(), result.schemaVersion() >= 2 ? "ATTEMPT#" + result.attemptId() : "FINAL").isEmpty()) throw new IllegalStateException("Old execution still has a result; retain for review");
            return false;
        }
        String resultHash = hash("result:v1", mapper.writeValueAsString(result));
        if (compacted(meta)) {
            var legacyJson = mapper.readValue(mapper.writeValueAsString(result), new tools.jackson.core.type.TypeReference<LinkedHashMap<String, Object>>() {});
            legacyJson.remove("requestKey");
            String legacyHash = hash("result:v1", mapper.writeValueAsString(legacyJson));
            if (!result.eventId().equals(value(meta, FENCE))
                    || !(resultHash.equals(value(meta, "result_hash")) || (result.schemaVersion() == 1 && legacyHash.equals(value(meta, "result_hash")))))
        ...
        var finalItem = read(result.deliveryId(), result.schemaVersion() >= 2 ? "ATTEMPT#" + result.attemptId() : "FINAL");
        if (!(result.schemaVersion() >= 2 ? Set.of("PENDING", "PUBLISHED").contains(value(finalItem, "publish_state"))
        ...
        for (var attempt : attempts) {
            ...
            if (result.schemaVersion() < 2 && attempt.containsKey("receipt_event_id") && !receiptIds.contains(value(attempt, "receipt_event_id")))
        ...
        if (result.schemaVersion() >= 2) {
            cache.complete(result.requestKey(), fingerprint(result.requestKey(), result.tenantId(), result.deliveryType(),
                    Boolean.TRUE.equals(meta.getOrDefault(FALLBACK_ALLOWED, AttributeValue.fromBool(false)).bool()), value(meta, PAYLOAD)), result.finalizedAt());
        }
        ...
        cache.removeSchedule(result.deliveryId());
        cache.removeSchedule(result.requestKey());
        return true;
    }
