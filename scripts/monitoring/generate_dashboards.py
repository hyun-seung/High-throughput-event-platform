...
d.text('결과 수신 · 폴백 · 외부 멱등성','허용 요청의 TCP 2차 접수와 재시도를 구현했습니다. 웹훅 수신·Attempt 결과 반영·실패 코드 재시도/전환을 구현했습니다. 주기 만료·고객 통지는 미구현입니다. 시뮬레이터 calls/effects 차이는 정상 성공률과 다르며 강제 실패도 함께 확인해야 합니다. Provider 성공 후 결과 저장 전 장애는 운영 확인 대상입니다.',0,21,24,3)
...
d.panel('웹훅 결과 처리 TPS',total(STAGE+'_count','stage="receipt_process",result="success"'),0,39,6,5,'stat','reqps',description='처리 관측 횟수이며 중복·폐기를 포함합니다. 새 Dispatch 빌드 적용 후 수집됩니다.')
d.panel('웹훅 성공 반영 TPS',total('delivery_outcomes_total','outcome="receipt_delivered"'),6,39,6,5,'stat','reqps',description='Attempt DELIVERED 저장 성공을 관측한 횟수입니다. 고객 통지 완료·고유 Delivery 수가 아니며 DB 응답 유실 시 과소 관측할 수 있습니다.')
d.panel('늦은 웹훅 폐기 관측',f'sum(increase(delivery_outcomes_total{{outcome="receipt_late_discarded"}}[{RANGE}]))',12,39,6,5,'stat')
d.panel('결과 트랜잭션 p95',quant('delivery_dynamodb_duration_seconds','operation="transact_write_items"'),18,39,6,5,'stat','s',description='논리 SDK 호출 지연입니다. 실제 WCU·항목 쓰기 수와 다릅니다.')
d.save()
...
d.text('후속 단계','웹훅 결과와 TCP 2차 로그를 연결할 수 있습니다. 만료·고객 통지는 후속이며 새 앱 빌드 적용이 필요합니다. 운영자가 실제 상태를 조회·조치하는 API는 별도 구현이 필요합니다.',0,22,24,3)
d.save()
