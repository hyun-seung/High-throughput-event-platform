...
public class CleanupWorker implements AutoCloseable {
    ...
    public CleanupWorker(CleanupRepository repository, DeliveryCompactor compactor, MeterRegistry meters,
        ...
        this(repository, compactor, meters, concurrency, batchSize, new FailureBackoff(Duration.ofSeconds(1), Duration.ofSeconds(30)));
    }
    public CleanupWorker(CleanupRepository repository, DeliveryCompactor compactor, MeterRegistry meters,
                         int concurrency, int batchSize, FailureBackoff backoff) {
    ...
    public synchronized void tick() {
        ...
        if (backoff.blocked()) { meters.counter("delivery.cleanup.events", "outcome", "poll_deferred").increment(); return; }
    ...
    private boolean processOne() {
        ...
        try {
            ...
            if (next.isEmpty()) { backoff.succeeded(ticket); return false; }
            claim = next.get();
            ...
            boolean unavailable = StorageFailure.unavailable(failure);
            if (unavailable) backoff.failed(ticket);
            if (claim != null) {
                try { repository.retry(claim); }
                catch (RuntimeException retryFailure) {
                    if (StorageFailure.unavailable(retryFailure)) { unavailable = true; backoff.failed(ticket); }
                    // SQL lease remains the recovery record.
                }
            }
            if (!unavailable) backoff.abandon(ticket);
            ...
            if (claim == null || unavailable) return false;
        } finally {
