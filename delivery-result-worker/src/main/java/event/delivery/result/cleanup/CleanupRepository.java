...
public class CleanupRepository {
    ...
    public Optional<Claim> claim() {
        return Objects.requireNonNull(tx.execute(status -> {
            ...
            jdbc.update("UPDATE delivery_cleanup_outbox SET lease_token = ?, lease_until = clock_timestamp() + interval '60 seconds' WHERE result_event_id = ?", token, row.get("result_event_id"));
    ...
    public void retry(Claim claim) {
        jdbc.update("""
                UPDATE delivery_cleanup_outbox SET next_attempt_at = clock_timestamp() + interval '30 seconds', lease_token = NULL, lease_until = NULL
                WHERE result_event_id = ? AND status = 'PENDING' AND lease_token = ?
